...
[GitHub Releases](https://github.com/pondashicom/note-pon/releases)から`NOTE-PON-Setup-0.2.2.exe`をダウンロードして実行します。
...
```text
publish\installer\NOTE-PON-Setup-0.2.2.exe
```
...
10. `A−`、`A＋`、「常に手前」が操作できることを確認します。
11. NOTE-PONを終了し、音量アップ／ダウン／ミュートキーがWindowsの音量操作へ戻ることを確認します。
